...
public class PhysicsTestVectors4 {
...
//        return null;
  ...
  public static void main(String[] args) {
    ...
    while (true) {
      ...
      if (m.vectors.size() > 0) {
        ArrayList<Vector> outVector = new ArrayList<>();
        for (int i1 = 0; i1 < m.vectors.size(); i1++) {
          ...
          RenderEngine
              .drawLine(null, mid.addi(vectorA).addi(vectorA.normalizei().perpi().scalei(-1000.0f)),
                  mid.addi(vectorA));
          for (int i2 = i1 + 1; i2 < m.vectors.size(); i2++) {
            Vector vectorB = m.vectors.get(i2);
            if (vectorA.subi(vectorB).dot(vectorB.normalizei()) < 0) {
              if (vectorA.dot(vectorB) > vectorA.magSqr()
                ...
                outVector.add(vectorB);
              } else {
                outVector.add(lineLine(vectorA, vectorA.addi(vectorA.perpi()), vectorB,
                    vectorB.addi(vectorB.perpi())));
              }
            } else {
              outVector.add(vectorA);
            }
        ...
        for (Vector v : outVector) {
          RenderEngine.setColor(null, Color.orange);
          RenderEngine.drawLine(null, inner.addi(v), inner);
          out.add(v);
        }
        out.scale(1.0f / outVector.size());
